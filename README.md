...
Temporary HTTP 429/5xx and transport failures are retried at most five times with short bounded delays, and both satellites are downloaded in parallel. If one satellite still fails, its payload is retried once after 30 seconds without discarding the other. Authentication failures and a missing/outdated satellite still fail without publishing. Public diagnostics distinguish timeouts, HTTP failures and connection failures using only fixed messages, never upstream URLs or payloads. The 15-minute GitHub schedule is best-effort: public repository cron jobs are often delayed by hours, so a local watchdog may dispatch this workflow when the published file grows stale. GitHub scheduling delays remain possible and require freshness monitoring for production operation.
