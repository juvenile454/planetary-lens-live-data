...
class FeedTest(unittest.TestCase):
    ...
    def test_utc_window_and_midnight_time(self):
        points, latest = feed.parse_csv(
            payload(
                ...
                row(acq_date="2026-09-05", acq_time="1159"),
                row(acq_time="1"),
                row(),
            ),
            "VIIRS_NOAA20_NRT",
    ...
    def test_failed_refresh_preserves_the_previous_file(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("sys.argv", ["build_feed.py", "--output", str(path)]),
                patch.dict("os.environ", {"FIRMS_MAP_KEY": ""}),
            ):
    ...
    def test_diagnostics_allow_known_reasons_without_exposing_untrusted_messages(self):
        self.assertEqual(
            "NASA CSV schema missing", feed.safe_failure_reason(ValueError("NASA CSV schema missing"))
        )
        secret = "a" * 32
    ...
    def test_download_all_retries_only_the_failed_source(self):
        ...
        def fake_download(_key, source):
            if source == "VIIRS_NOAA20_NRT":
                state["n20"] += 1
                if state["n20"] == 1:
                    raise feed.DownloadError(source, "transport code 56", True)
                return n20
            return n21
